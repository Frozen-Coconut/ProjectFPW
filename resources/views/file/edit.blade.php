...
<form action="{{route('file_edit_post')}}" class="w-full h-screen flex flex-col p-8 overflow-y-auto" method="POST">
    ...
    <div class="flex justify-end mt-6">
        <button href="{{route('file_edit_post')}}" name="save" class="inline-block px-6 py-2.5 bg-blue-600 text-white font-medium text-xs leading-tight uppercase rounded shadow-md hover:bg-blue-700 hover:shadow-lg focus:bg-blue-700 focus:shadow-lg focus:outline-none focus:ring-0 active:bg-blue-800 active:shadow-lg transition duration-150 ease-in-out">Simpan File</button>
    </div>
    <script>
        $('#text').linedtextarea();
    </script>
